...
window.x_init_hook_team = function() {
    ...
    function() {
        var n = parseInt(jQuery(this).attr('value'), 10);					//转为10进制数字,获取购买数量。
        var per = parseInt(jQuery('#deal-per-number').attr('value'), 10);	//获取每次最多购买数量
        //控制最大购买数量
		if (n > per && per > 0) {
            n = per
        }
...
window.x_init_hook_order = function() {
    jQuery('form[tname="order-pay-form"]').bind('submit', 
    function() {
    	var enough_money = jQuery('#enough_money').val();
    	
    	if(enough_money == 'yes'){
    		if(jQuery('#funds').attr("checked")){
    			jQuery('form[tname="order-pay-form"]').attr('target','_self');
    		}else if(jQuery('#check_online').attr("checked") ){
    		...
    		if(jQuery('#check_online').attr("checked") ){
        		X.get(WEB_ROOT + '/ajax/order.php?action=dialog&id=' + jQuery(this).attr('sid'));
        	}else{
        		jQuery('form[tname="order-pay-form"]').attr('target','_self');
        		return true;
...
//   		if(jQuery('.yinh').attr("checked") || jQuery('#check_cash').attr("checked")){
//   			alert('A');
//   			if(jQuery('#check_cash').attr("checked")){
//   				jQuery('form[tname="order-pay-form"]').attr('target','_self');
//   			}
//    		return true;
//    	}else{
...
X.manage.usermoney = function() {
    ...
    var remark = jQuery('#user-dialog-input-pay-remark').val();
    if (uid && money && (!ask || confirm(ask))) return ! X.get(WEB_ROOT + '/ajax/manage.php?action=usermoney&id=' + uid + '&money=' + encodeURIComponent(money) +'&remark=' + encodeURIComponent(remark)) 
};
X.manage.orderexpress = function() {
    ...
    var tel = jQuery('#tel').attr('value');
    var oldexp = jQuery('#oldexp').attr('value');
    if (oid) return ! X.get(WEB_ROOT + '/ajax/manage.php?action=orderexpress&id=' + oid + '&eid=' + eid+ '&tel=' + tel + '&oldexp=' + oldexp+ '&nid=' + encodeURIComponent(nid))
};
...
/*order state add by herui 2011-1-11*/
X.manage.changOrderState = function() {
	var rid = jQuery('#order-dialog-OrderState-id').val();
	var verify_info = $('#verify_info').val();
	if(rid=='notpass' && verify_info==''){
		alert('必须填写审核未通过原因');
		return ;
	}
	var oid = jQuery('#dialog-order-id').attr('oid');
	var osta = jQuery('#order-dialog-OrderState-id').attr('osta');
	if(rid == osta){
		return;
	}
	verify_info = encodeURIComponent(verify_info);
	if(oid&&rid&&osta) return !X.get(WEB_ROOT + '/ajax/manage.php?action=orderStat&id='+oid+'&rid='+rid+'&osta='+osta+'&verify_info='+verify_info);
};
